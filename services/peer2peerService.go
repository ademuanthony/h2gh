...
func (this Peer2PeerService) CreatePayment(fromMemberId int64) error {
	...
	err = this.O.QueryTable(new(models.Queue)).Filter("amount", 1000).Filter("Member__status", models.StatusActive).Filter("status", models.StatusPending).RelatedSel().OrderBy("sort_order").One(&bonusQueue)
	if err != nil{
